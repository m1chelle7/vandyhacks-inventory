...
const InventoryTable: React.FC<Props> = ({ colorScheme }) => {
  // add quantity information, if opened
  const [openedAddQuant, setOpenedAddQuant] = useState(false);

  // delete quantity infromation, if opened
  const [openedDelQuant, setOpenedDelQuant] = useState(false);

  // confirm to add quantity
  const [checkedIncreaseInventory, setCheckedIncreaseInventory] =
    useState(false);

  // data displayed in the inventory table
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost:3001/api/data/inventory"
        );
  ...
  return (
    ...
    >
      <Group gap={7} mb={6} align="center">
        ...
        <Modal.Root
          opened={openedAddQuant}
          onClose={() => setOpenedAddQuant(false)}
          centered
        >
          <Modal.Overlay />
          <Modal.Content>
            ...
            <Modal.Body>
              <Stack mb={15}>
                ...
                <Checkbox
                  ...
                  onChange={(event) =>
                    setCheckedIncreaseInventory(event.currentTarget.checked)
                  }
                  color="black"
                  required
                />
        ...
        {/* 'delete from quantity' modal */}
        <Modal.Root
          opened={openedDelQuant}
          onClose={() => setOpenedDelQuant(false)}
          centered
        >
          <Modal.Overlay />
          <Modal.Content>
        ...
        {/* 'delete from quantity' hover text */}
